...
class DYFilter : public edm::stream::EDFilter<> {
   public:
      explicit DYFilter(const edm::ParameterSet&);
   private:
      virtual bool filter(edm::Event&, const edm::EventSetup&) override;
      edm::EDGetTokenT<std::vector<reco::GenParticle>> genParticleToken;
};

DYFilter::DYFilter(const edm::ParameterSet& iConfig)
{
   genParticleToken = consumes<std::vector<reco::GenParticle>>(iConfig.getParameter<edm::InputTag>("genParticleCollection")); 
}

bool DYFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
